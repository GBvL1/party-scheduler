...
type Step = "boot" | "q1" | "q2" | "q3" | "name" | "briefing" | "aborted";

// null        = normal
// "crashing"  = glitch-crash animation playing on content wrapper
// "black"     = fixed full-screen black overlay covering everything
type CrashPhase = null | "crashing" | "black";
...
export default function MissionPage() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setBriefingCountdown((c) => {
        if (c <= 1) {
          clearInterval(id);
          // Phase 1: violent glitch animation on content (1.6s)
          setCrashPhase("crashing");
          // Phase 2: fixed full-screen black overlay — covers crosshair, overlays, everything
          setTimeout(() => setCrashPhase("black"), 1600);
          return 0;
        }
        return c - 1;
      });
  ...
  async function handleAccept() {
    ...
    try {
      ...
      // Suppress unused warning — name stored server-side, we just need success
      void data;
      setStepGlitch(true);
  ...
  return (
    <>
      {/* Fixed full-screen blackout — sits above ALL layout overlays (z-index > 9999).
          Activated after the glitch-crash animation completes. Stays forever. */}
      {crashPhase === "black" && (
      ...
      <main className={`min-h-screen flex flex-col items-center justify-center px-4 py-12 ${booted ? "crt-boot" : "opacity-0"}`}>
        <div
          className={[
            ...
            crashPhase === "crashing" ? "glitch-crash" : "",
          ].filter(Boolean).join(" ")}
        >

          {/* CLASSIFICATION HEADER — hidden during briefing for full immersion */}
          {step !== "briefing" && (
